...
public interface RoleDBRepository extends CrudRepository<RoleDB, Long> {
    ...
    List<RoleDB> findByLocalUnitDB_LocalUnitID(Long localUnitID);

    @Query("select r from RoleDB r where r.localUnitDB.localUnitID is null")
    List<RoleDB> findByLocalUnitDB_LocalUnitIDNull();
