...
public interface EventSessionDBRepository extends CrudRepository<EventSessionDB, Long> {
    @Query("select e from EventSessionDB e where e.eventDB.localUnitDB.localUnitID = ?1 and e.start >= ?2")
    ...
    @Query("select e from EventSessionDB e where e.eventDB.localUnitDB.localUnitID = ?1 and e.start >= ?2 and e.end <= ?3")
